    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl font-extrabold leading-tight">
          Tunisie, Maroc et Égypte
        </h1>
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-[hsl(var(--muted))]">
          Un investisseur qui étudie l&apos;Afrique du Nord ne choisit pas un pays dans
          l&apos;absolu : il arbitre entre plusieurs. Ces indicateurs situent la Tunisie
          face à ses deux voisins les plus comparables.
        </p>
      </header>
      ...
      <div className="surface-card flex flex-wrap items-start gap-3 p-4">
        ...
        <p className="flex-1 text-xs leading-relaxed text-[hsl(var(--muted))]">
          Toutes les valeurs proviennent des <strong className="text-[hsl(var(--foreground))]">
          World Development Indicators de la Banque mondiale</strong> (licence CC BY 4.0),
          reproduites sans retraitement. Chaque pays est présenté à sa dernière année
          publiée — elles peuvent différer d&apos;un pays à l&apos;autre, la source ne les
          publiant pas au même rythme. Aucune valeur n&apos;est estimée ni interpolée.
        </p>
        {total > 0 && <Badge variant="neutre">{total} indicateurs</Badge>}
      </div>
        ...
        return (
          <Card key={groupe.secteurId}>
            <CardHeader className="flex-row items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base">{groupe.secteur}</CardTitle>
                <CardDescription>
                  {groupe.indicateurs.length} indicateur(s) comparable(s)
                </CardDescription>
              </div>
            </CardHeader>
              ...
              {groupe.indicateurs.map((indicateur) => {
                ...
                return (
                  <div key={indicateur.id} className="rounded-[var(--radius-control)] border border-[hsl(var(--border))] p-4">
                    <p className="text-sm font-medium leading-tight">{indicateur.indicateur}</p>
                    <p className="mt-0.5 text-xs text-[hsl(var(--muted))]">
                      {indicateur.unite ?? ''}{indicateur.annee ? ` · ${indicateur.annee}` : ''}
                    </p>
                      ...
                      <BarChart data={series} margin={{ top: 4, right: 4, bottom: 0, left: -18 }}>
                        ...
                        <Bar dataKey="valeur" name={indicateur.indicateur} radius={[5, 5, 0, 0]} maxBarSize={38}>
                          {series.map((point) => (
                            <Cell
                              key={point.pays}
                              fill={PAYS.find((p) => p.libelle === point.pays)?.couleur}
                            />
                          ))}
                        </Bar>
                      </BarChart>
                    ...
                    <ul className="mt-2 space-y-1">
                      {PAYS.map(({ cle, libelle, couleur }) => (
                        <li key={cle} className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-[hsl(var(--muted))]">{libelle}</span>
                          <span className="tabular ml-auto font-semibold">
                            {indicateur[cle] == null ? 'n.d.' : formatNombre(Number(indicateur[cle]), 2)}
                          </span>
                        </li>
                      ))}
                    </ul>
