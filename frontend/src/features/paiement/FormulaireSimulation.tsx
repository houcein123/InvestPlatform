...
export function FormulaireSimulation({ sectorId, onPaiementConfirme, onErreur }: Props) {
  ...
  async function soumettre(evenement: React.FormEvent) {
    ...
    if (!EMAIL.test(email)) {
      setErreur(t.paiement.emailInvalide);
      return;
    }
    setErreur('');
    setEnCours(true);
    ...
    try {
      ...
      onErreur(probleme instanceof Error ? probleme.message : t.paiement.validationEchouee);
    } finally {
      setEnCours(false);
    }
  }
      ...
      <div className="flex items-start gap-3 rounded-[var(--radius-control)] border border-[hsl(var(--warning)/0.35)] bg-[hsl(var(--warning)/0.08)] px-4 py-3">
        ...
        <div className="text-xs leading-relaxed">
          <p className="text-sm font-semibold text-[hsl(var(--foreground))]">
            {t.paiement.demoTitre}
          </p>
          <p className="mt-1 text-[hsl(var(--muted))]">
            {t.paiement.demoAvant}
            ...
            {t.paiement.demoApres}
          </p>
        </div>
      </div>

      <div className="space-y-1.5">
        <Label htmlFor="email-facturation">{t.paiement.adresseFacturation}</Label>
        <Input
          id="email-facturation"
          type="email"
          autoComplete="email"
          placeholder={t.commun.emailExemple}
          value={email}
          onChange={(evenement) => setEmail(evenement.target.value)}
          required
        />
        <p className="flex items-center gap-1.5 text-xs text-[hsl(var(--muted))]">
          <Info className="size-3" /> {t.paiement.adresseFacturationAide}
        </p>
        <ErreurChamp>{erreur}</ErreurChamp>
      </div>

      <div className="space-y-1.5">
        <Label htmlFor="nom-facturation">{t.paiement.nomFacturation}</Label>
        <Input
          id="nom-facturation"
          autoComplete="name"
          placeholder={t.paiement.nomFacturationExemple}
          value={nom}
