...
async function generateNarratives(data, { onSection, onAttente } = {}) {
    ...
    for (let i = 0; i < SECTION_KEYS.length; i++) {
        ...
        // tentative de reprise. Sans quota recent, l'appel est immediat.
        await groqService.espacerSiQuotaRecent((secondes) => {
            if (onAttente) {
                onAttente(`Quota de redaction atteint — reprise dans ${secondes} s`);
            }
        });
...
function tailleFichierDe(pdf) {
    try {
        return fs.statSync(pdf.path).size;
    } catch (err) {
        console.warn("⚠️  Taille du PDF non mesurable :", err.message);
        return null;
    }
}

/**
 * Aligne le volume annoncé au catalogue sur le dernier rapport produit.
 *
 * `secteurs.nombre_pages` sert de promesse commerciale : c'est le chiffre lu
 * sur la carte du catalogue, sur le récapitulatif de commande et sur l'aperçu
 * gratuit (« 2 pages sur N »). Le laisser à sa valeur de départ revenait à
 * annoncer 14 pages pour un document qui en fait souvent davantage. On le
 * recale donc sur le décompte réel, sans jamais bloquer la livraison.
 */
async function synchroniserPagesSecteur(secteur, nombrePagesReel) {
    if (!nombrePagesReel || nombrePagesReel === secteur.nombre_pages) return;
    try {
        await pool.query("UPDATE secteurs SET nombre_pages = $1 WHERE id = $2",
            [nombrePagesReel, secteur.id]);
    } catch (err) {
        console.warn("⚠️  Volume annoncé du secteur non mis à jour :", err.message);
    }
}
...
async function persistRapport({ utilisateurId, achatId, secteur, pdf, narratives }) {
    try {
        // La taille est mesurée sur le fichier réellement écrit, pas estimée.
        // Elle restait NULL en base : le panneau d'administration ne pouvait
        // donc afficher ni le poids d'un rapport ni le volume total produit.
        const tailleFichier = tailleFichierDe(pdf);

        // Le nombre de pages enregistré est celui du document RÉELLEMENT
        // produit (compté par le générateur), et non les 14 pages annoncées au
        // catalogue : un rapport dont les textes sont longs en fait davantage,
        // et le client doit lire dans son espace le volume qu'il a reçu.
        const nombrePages = pdf.nombrePages || secteur.nombre_pages;

        const { rows } = await pool.query(
            `INSERT INTO rapports
                (utilisateur_id, secteur_id, titre, chemin_fichier, taille_fichier,
                 nombre_pages, statut, contenu_ia, date_generation)
             VALUES ($1,$2,$3,$4,$5,$6,'genere',$7, CURRENT_TIMESTAMP)
             RETURNING id`,
            [
                utilisateurId,
                secteur.id,
        ...
        await synchroniserPagesSecteur(secteur, pdf.nombrePages);
...
async function updateReportNarratives(rapportId, narrativesModifiees) {
    ...
    await pool.query(
        // La reconstruction produit un NOUVEAU fichier : sa taille ET son
        // nombre de pages changent avec les textes corrigés, et doivent suivre,
        // sinon le panneau affiche les caractéristiques de l'ancien document.
        `UPDATE rapports
            SET contenu_ia = $1, chemin_fichier = $2, taille_fichier = $3,
                nombre_pages = COALESCE($4, nombre_pages),
                date_generation = CURRENT_TIMESTAMP
          WHERE id = $5`,
        [JSON.stringify(narratives), pdf.url, tailleFichierDe(pdf), pdf.nombrePages || null, rapportId]
    );

    await synchroniserPagesSecteur(data.secteur, pdf.nombrePages);
