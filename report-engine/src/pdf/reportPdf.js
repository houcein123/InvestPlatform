...
const {
    drawLineChart, drawDonutChart, drawHorizontalBars, drawComparisonBars,
} = require("./charts");
const {
    dessinerBandeauCouverture, dessinerIllustrationSection, photosSecteur, ligneCredit,
} = require("./visuels");
...
/**
 * Courbes d'evolution, sur deux colonnes.
 *
 * Complementaires des barres et non redondantes : les barres comparent des
 * annees entre elles, la courbe montre la FORME de la trajectoire. Une
 * accelaration, un palier ou un retournement se lisent ici et pas la.
 *
 * Seules les series d'au moins trois points y figurent : sur deux points, une
 * « courbe » est un segment, et un segment ne montre aucune inflexion.
 */
function drawTrajectoires(doc, rows) {
    const lignes = rows.filter((row) => {
        const serie = extractSeries(row);
        return serie.values.filter((v) => v !== null && v !== undefined && !isNaN(v)).length >= 3;
    }).slice(0, 2);
    ...
    lignes.forEach((row, i) => {
        ...
        doc.roundedRect(x, rowY, cardW, cardH, 6).fillAndStroke(COLORS.white, COLORS.border);
...
function renderChiffresClesSection(doc, report) {
    ...
    drawTrajectoires(doc, misesEnAvant);
...
/**
 * Deux graphiques cote a cote dans une bande, avec repli propre.
 * Facteur commun aux sections zones, acteurs et comparatif.
 */
function bandeDeuxGraphiques(doc, hauteur, dessiner) {
    const startX = doc.page.margins.left;
    const width = contentWidth(doc);
    const gap = 16;
    const cardW = (width - gap) / 2;

    ensureSpace(doc, hauteur + 16);
    const rowY = doc.y;

    [0, 1].forEach((colonne) => {
        const x = startX + colonne * (cardW + gap);
        doc.roundedRect(x, rowY, cardW, hauteur, 6).fillAndStroke(COLORS.white, COLORS.border);
        dessiner(colonne, { x: x + 12, y: rowY + 10, largeur: cardW - 24, hauteur: hauteur - 20 });
    });

    doc.y = rowY + hauteur + 14;
    doc.x = startX;
}

/** Repartition des zones par type, et classement par superficie. */
function drawZonesGraphiques(doc, zones) {
    const liste = zones || [];
    ...
    const parSuperficie = liste
        .filter((zone) => zone.superficie_km2 != null)
        .map((zone) => ({ nom: zone.nom, valeur: Number(zone.superficie_km2) }))
        .sort((a, b) => b.valeur - a.valeur);

    // Un seul type et aucune superficie : deux graphiques diraient moins que
    // la liste elle-meme. On s'abstient plutot que de meubler.
...
/** Origine des acteurs, et classement par chiffre d'affaires. */
function drawActeursGraphiques(doc, acteurs) {
    const liste = acteurs || [];
    if (liste.length === 0) return;
    ...
    const parChiffre = liste
        .filter((a) => a.chiffre_affaires != null)
        .map((a) => ({ nom: a.nom, valeur: Number(a.chiffre_affaires) }))
        .sort((a, b) => b.valeur - a.valeur);
...
function renderZonesSection(doc, zones) {
    ...
    if (zones && zones.some((z) => z.type === "zone_franche")) {
        ...
        doc.font("Helvetica").fontSize(8).fillColor(COLORS.gold)
            // Puce « • » et non un carré plein : les polices standard du PDF
            // n'encodent que le jeu WinAnsi, où le caractère « ▮ » ressortait
            // en charabia au bas de la section.
...
function renderBenchmarkTable(doc, benchmarks) {
    ...
    // Barres groupees sur les trois premiers indicateurs : un ecart entre pays
    // se percoit d'un coup d'oeil la ou un tableau demande une lecture ligne a
    // ligne. Un pays sans valeur reste VIDE, avec la mention « n. d. » —
    ...
    if (graphables.length > 0) {
        ...
        ensureSpace(doc, carteH + 16);
        const rangeeY = doc.y;

        graphables.forEach((ligne, i) => {
            const x = startX + i * (carteW + espace);
            doc.roundedRect(x, rangeeY, carteW, carteH, 6).fillAndStroke(COLORS.white, COLORS.border);
...
function renderSourcesSection(doc, report) {
    ...
    // Les licences des photographies (CC BY, CC BY-SA, CC0) autorisent l'usage
    // commercial de ce rapport à la condition de citer l'auteur : la liste
    // ci-dessous est donc une obligation, pas une politesse.
    ...
    if (photos.length > 0) {
        ...
        doc.font("Helvetica-Oblique").fontSize(8.5).fillColor(COLORS.textMuted)
            .text("Photographies documentaires du secteur en Tunisie, reproduites sous leur licence "
                + "d'origine ; leurs auteurs ne sont pas associés à ce rapport.",
                startX, doc.y, { width, align: "justify", lineGap: 3.5 });
        doc.moveDown(0.6);
        doc.x = startX;
    }
...
/**
 * Écrit le nombre de pages dans la chip laissée vide par la couverture.
 *
 * Le décompte EXACT n'est connu qu'une fois toutes les sections rendues : le
 * texte rédigé déborde librement, un rapport ne fait donc pas toujours le même
 * nombre de pages. On revient donc sur la page 1 à la fin — d'où `bufferPages`.
 */
function ecrireNombrePages(doc, chip, nombrePages) {
    if (!chip) return;
    doc.switchToPage(chip.pageIndex);
    doc.font("Helvetica-Bold").fontSize(11.5).fillColor(COLORS.primaryDark)
        .text(String(nombrePages), chip.x, chip.y, { width: chip.largeur, align: "center", lineBreak: false });
}

/**
 * @param {{isPreview?: boolean, nombrePages?: number|null}} options
 *        nombrePages connu d'avance (aperçu) ; sinon la chip reste vide et
 *        `ecrireNombrePages` la remplit à la fin du rendu.
 * @returns {{chipPages: object}} position de la chip « Pages »
 */
function buildCoverPage(doc, report, { isPreview = false, nombrePages = null } = {}) {
    const w = doc.page.width;
    const secteur = report.secteur || {};

    // Bandeau supérieur — photographie du secteur si l'éditeur en a déposé une
    // dans assets/secteurs/, sinon emblème vectoriel propre au secteur. Voir
    // pdf/visuels.js : rien n'est téléchargé, rien n'est emprunté.
    ...
    doc.y = 330;
    // La valeur « Pages » peut être NULLE ici : c'est le décompte réel du
    ...
    const pageIndexCouverture = doc.bufferedPageRange().start;
    const chipPages = { pageIndex: pageIndexCouverture, x: PAGE_MARGIN, y: chipRowY + 27, largeur: chipW };
...
/**
 * Sections illustrées par une photographie, et rang de l'image utilisée.
 *
 * La couverture prend la photographie n° 1. Deux sections seulement en portent
 * une autre : la présentation générale, où elle donne un visage au secteur, et
 * les zones géographiques, où elle montre un lieu réel. Illustrer chaque
 * section transformerait un rapport d'investissement en brochure.
 */
const ILLUSTRATIONS = {
    introduction: 2,
    zones: 3,
 ...
 */
function finalize(doc, filePath, filename, nombrePages) {
    return new Promise((resolve, reject) => {
        const stream = fs.createWriteStream(filePath);
        stream.on("finish", () => resolve({
            filename, path: filePath, url: `/reports/${filename}`, nombrePages,
        }));
 ...
 * @returns {Promise<{filename: string, path: string, url: string, nombrePages: number}>}
 *          `nombrePages` est le décompte RÉEL du document produit, pas la
 *          valeur annoncée au catalogue.
 */
async function generateReportPDF(report) {
    ...
    // ---- Page 1 : couverture --------------------------------------------
    // Le nombre de pages n'est pas encore connu : la chip correspondante est
    // laissée vide et remplie plus bas, une fois le document complet.
    const { chipPages } = buildCoverPage(doc, report);
    ...
    SECTION_CATALOG.forEach((section, i) => {
        ...
        if (ILLUSTRATIONS[section.key]) {
            dessinerIllustrationSection(doc, secteur, { rang: ILLUSTRATIONS[section.key], hauteur: 155 });
        }
        RENDERERS[section.key](doc, report);
    });

    // ---- Rétro-remplissage : sommaire, puis décompte de pages ------------
    fillTOCPageNumbers(doc, tocEntries);
    const nombrePages = doc.bufferedPageRange().count;
    ecrireNombrePages(doc, chipPages, nombrePages);
    doc.flushPages();

    return finalize(doc, filePath, filename, nombrePages);
}
...
async function generatePreviewPDF(report) {
    ...
    // dernière génération (secteurs.nombre_pages, mis à jour par le moteur).
    const pagesRapport = secteur.nombre_pages || SECTION_CATALOG.length + 2;
    buildCoverPage(doc, report, { isPreview: true, nombrePages: pagesRapport });
    ...
    return finalize(doc, filePath, filename, doc.bufferedPageRange().count);
}
